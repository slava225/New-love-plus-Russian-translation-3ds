<div align="center">

<img src="assets/new-love-plus-russian-translation-banner.jpg" alt="New Love Plus+ — русский перевод для Nintendo 3DS" width="100%">

# New Love Plus+ для Nintendo 3DS — русский перевод

**Неофициальный перевод игры New Love Plus+ на русский язык.**

> 🚧 **Проект находится в разработке. Перевод ещё не завершён.**

</div>

## О проекте

Цель проекта — подготовить полный и качественный русский перевод **New Love Plus+** для **Nintendo 3DS**.

Сейчас репозиторий находится на начальном этапе. Файлы перевода, подробное описание, инструкции и другая информация будут добавляться постепенно.

> 🤖 **На текущем этапе черновой перевод создаётся преимущественно с помощью искусственного интеллекта.** Первичная вычитка была проведена автором проекта с помощью **Gemini 3.5 Flash**. Перевод всё ещё требует обязательной ручной проверки. Проекту нужен человек, который хорошо знает японский язык и сможет сверять перевод с оригиналом, исправлять неточности и помогать с естественной адаптацией текста.

## Текущий статус

- [x] Создан репозиторий проекта
- [ ] Добавлены файлы перевода
- [ ] Указан процент готовности
- [ ] Добавлена инструкция по установке
- [ ] Проведено тестирование перевода
- [ ] Опубликован первый релиз

<!-- Здесь можно указать текущий процент перевода и дату обновления. -->

## Что планируется перевести

- сюжет и диалоги;
- меню и элементы интерфейса;
- системные сообщения и подсказки;
- другой внутриигровой текст.

<!-- При необходимости дополни или измени этот список. -->

## Установка

Инструкция по установке появится позже.

<!-- Здесь можно добавить требования, порядок установки патча и список совместимых версий игры. -->

## Как помочь проекту

В первую очередь проекту нужен участник, хорошо владеющий **японским языком**, для проверки ИИ-перевода, контекста и особенностей речи персонажей.

Также в будущем потребуется помощь с редактированием русского текста, поиском ошибок и тестированием перевода в игре.

## Благодарности

Огромная благодарность команде [LovePlusProject](https://github.com/LovePlusProject) и всем участникам связанных проектов:

- [NLPPATCH](https://github.com/LovePlusProject/NLPPATCH) — за незавершённый английский перевод **New Love Plus+**, технические исследования, документацию и основу для дальнейшей работы с игрой;
- [NLPPCTR](https://github.com/LovePlusProject/NLPPCTR) — за моды, текстуры, шаблоны и материалы для работы с графикой игры;
- разработчикам инструментов [NLPUnpacker](https://github.com/LovePlusProject/NLPUnpacker), [NLPTextTool](https://github.com/LovePlusProject/NLPTextTool), [nlpp-tools](https://github.com/LovePlusProject/nlpp-tools), [Trb2xlsx](https://github.com/LovePlusProject/Trb2xlsx), [png2texi](https://github.com/LovePlusProject/png2texi), [nlpp-fmt](https://github.com/LovePlusProject/nlpp-fmt) и [NLPLineWrapper](https://github.com/Aikin-Ai/NLPLineWrapper);
- всем переводчикам, программистам, художникам, тестировщикам и участникам сообщества, которые помогали исследовать игру и делились своими наработками.

Полный список участников оригинального английского проекта можно посмотреть в [официальных благодарностях NLPPATCH](https://github.com/LovePlusProject/NLPPATCH/issues/1).

## Важная информация

Это неофициальный фанатский проект. Репозиторий не предназначен для распространения образа игры или других защищённых авторским правом файлов. Для использования будущего патча потребуется собственная копия игры.

**New Love Plus+** и связанные с ней материалы принадлежат их правообладателям.

## Автор

Проект ведёт [slava225](https://github.com/slava225).

<!-- Здесь можно добавить контакты, участников команды и ссылки на обсуждение проекта. -->
